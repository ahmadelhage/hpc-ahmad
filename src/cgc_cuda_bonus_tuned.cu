...
// ─────────────────────────────────────────────────────────────────────────────
// VARIANT A: cluster sum using global atomics only (baseline reference)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// VARIANT B: cluster sum using shared memory (reduces global atomic contention)
//   Each block accumulates into shared memory, then flushes once to global.
...
//   Threads within a warp exchange values in registers (no memory needed).
//   Adjacent threads with the same cluster combine their values via shuffle,
//   then only one thread per group writes to global memory.
//   This minimises global atomic traffic to at most one per warp-group per cluster.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// KERNEL 2: divide sums by counts (one thread per cluster)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// KERNEL 3: partial row-label distances (one thread per row x candidate_label)
// ─────────────────────────────────────────────────────────────────────────────
    ...
    const double* cluster_avg, double* partial_dist)
{
    ...
    partial_dist[idx] = dist;
}

// ─────────────────────────────────────────────────────────────────────────────
// KERNEL 4: update column labels (one thread per local column)
// ─────────────────────────────────────────────────────────────────────────────
...
//   Picks the fastest variant + block size combination automatically.
// ─────────────────────────────────────────────────────────────────────────────
    ...
    double* d_partial_dist, int* d_cols_updated)
{
    ...
    // ── Tune cluster_sum: compare all three variants at all block sizes ───────
    {
        float best_ms = 1e9f;
...
// ─────────────────────────────────────────────────────────────────────────────
// Helper: launch whichever cluster_sum variant was selected by auto-tuner
// ─────────────────────────────────────────────────────────────────────────────
    ...
    cudaStream_t stream)
{
    ...
    switch (cfg.sum_variant) {
        case SumVariant::ATOMIC:
            kernel_cluster_sum_atomic<<<blocks, cfg.cluster_sum, 0, stream>>>(
                num_rows, local_cols, num_col_labels, num_clusters,
                d_matrix, d_row_labels, d_col_labels, d_global_sum, d_global_count);
            break;
...
// ─────────────────────────────────────────────────────────────────────────────
// Main loop: MPI + CUDA + shared mem + warp shuffle + streams + auto-tuning
// ─────────────────────────────────────────────────────────────────────────────
void cluster_cuda_bonus(
    int rank, int nprocs,
    ...
    int max_iterations)
{
    ...
    // ── Pinned host buffers (required for async transfers) ────────────────────
    ...
    CUDA_CHECK(cudaMemcpy(local_col_labels, d_col_labels,
